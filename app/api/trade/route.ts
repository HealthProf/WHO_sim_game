...
export async function GET() {
  ...
  const offers = await db.query.regionTradeOffers.findMany({
    where: eq(regionTradeOffers.sessionId, sessionId),
    orderBy: desc(regionTradeOffers.createdAt),
    limit: 30,
  });
...
export async function POST(req: NextRequest) {
  ...
  const buyerState = await db.query.modelState.findFirst({ where: and(eq(modelState.sessionId, sessionId), eq(modelState.regionId, fromTeam.regionId)) });
  ...
  const [offer] = await db
    ...
    .values({ sessionId, fromTeamId: fromTeam.id, toTeamId: toTeam.id, resourceType, amount, pricePerUnit, totalPrice })
    .returning();
...
export async function PATCH(req: NextRequest) {
  ...
  const offer = await db.query.regionTradeOffers.findFirst({ where: and(eq(regionTradeOffers.sessionId, sessionId), eq(regionTradeOffers.id, offerId)) });
  if (!offer || offer.status !== "pending") return NextResponse.json({ error: "Offer not found or already resolved" }, { status: 404 });
  if (offer.toTeamId !== actor!.teamId) return NextResponse.json({ error: "This offer isn't addressed to your region" }, { status: 403 });
  ...
  const buyerTeam = await db.query.teams.findFirst({ where: and(eq(teams.sessionId, sessionId), eq(teams.id, offer.fromTeamId)) });
  const sellerTeam = await db.query.teams.findFirst({ where: and(eq(teams.sessionId, sessionId), eq(teams.id, offer.toTeamId)) });
  ...
  const sellerDeducted = await tryDeductRegionField(sessionId, sellerTeam.regionId, resourceField, offer.amount);
  if (!sellerDeducted) {
  ...
  const buyerDeducted = await tryDeductRegionField(sessionId, buyerTeam.regionId, "fundRemaining", offer.totalPrice);
  if (!buyerDeducted) {
    await creditRegionField(sessionId, sellerTeam.regionId, resourceField, offer.amount); // compensate the seller deduction above
    await db.update(regionTradeOffers).set({ status: "rejected" }).where(eq(regionTradeOffers.id, offerId));
    return NextResponse.json({ error: `${buyerTeam.regionId} no longer has enough funds for this trade.` }, { status: 400 });
  }

  await creditRegionField(sessionId, buyerTeam.regionId, resourceField, offer.amount);
  await creditRegionField(sessionId, sellerTeam.regionId, "fundRemaining", offer.totalPrice);
  ...
  const [updatedBuyer, updatedSeller] = await Promise.all([
    db.query.modelState.findFirst({ where: and(eq(modelState.sessionId, sessionId), eq(modelState.regionId, buyerTeam.regionId)) }),
    db.query.modelState.findFirst({ where: and(eq(modelState.sessionId, sessionId), eq(modelState.regionId, sellerTeam.regionId)) }),
  ]);
  if (updatedBuyer) await db.insert(modelStateHistory).values({ sessionId, regionId: buyerTeam.regionId, day: updatedBuyer.day, snapshotJson: updatedBuyer, reason });
