...
**1. Fix `app/layout.tsx`'s site metadata — do this in Phase 2, or sooner.**

`app/layout.tsx:16-19` still ships create-next-app's scaffolding placeholders,
untouched since the project was generated:
...
This is the **only** place a title or description is set anywhere in the app —
verified, nothing overrides it (every page is `"use client"`, and client components
can't export `metadata`). So it applies to every route: dashboard, login, projector
display, all of them. It is what currently appears in the browser tab, in bookmarks,
in Google results, in the name iOS suggests for a home-screen icon, and — the one
that matters most, given the demo is meant to be found from LinkedIn — in the link
preview card when the URL is pasted into LinkedIn, Slack, or iMessage.

Replace with (copy drawn from `README.md:1-7`, the authoritative description):

```ts
export const metadata: Metadata = {
  metadataBase: new URL("https://who-sim-game.vercel.app"),  // required for relative OG image URLs
  title: {
    default: "Operation Veiled Horizon — WHO Pandemic Response Simulation",
    template: "%s — Operation Veiled Horizon",   // per-page titles, e.g. "Situation Room — …"
  },
  description:
...
Set `metadataBase` from an env var rather than the hardcoded URL if the deployment
domain isn't settled. The `openGraph` block is what makes the LinkedIn/Slack preview
card render properly instead of falling back to bare title text; add an
`openGraph.images` entry once Phase 6's icon assets exist (6a.4).
...
Two Phase 6 items are pulled forward and must not wait, both specified in 6a.2:
the site-metadata fix in `app/layout.tsx` (Phase 2 — replacement copy is written out
in full, don't re-invent it) and building all new Phase 3/4 UI mobile-first.
