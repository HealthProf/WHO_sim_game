...
LLM-driven autoplay (interface only, scripted implementation), and email delivery of
any kind.
...
## Appendix — assumptions made without an answer

These were put to Tim at planning time and went unanswered; the plan proceeds on the
recommended option and each is cheap to reverse.

1. **Test harness:** Vitest + real local Postgres (§6.1). Fallback: `tsx` scripts.
2. **Existing Neon data:** treated as disposable — wipe, `db:push`, re-seed static
   content. The repo already ships an instructor reset button that wipes everything,
   so nothing durable is at risk. If any deployed session must be preserved, a
   one-time backfill stamping every row with a single legacy session id is the
   alternative — say so before Phase 1 starts, not after.
