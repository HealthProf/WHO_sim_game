...
export const eventSeed: EventSeed[] = [
  ...
  // --- Social-metric consequence arcs (public trust, population happiness,
  // political tension) — each a 3-stage warning/escalation/crisis arc per
  // region, dispatched by the instructor to whichever region(s) currently
  // cross that stage's threshold (see lib/event-targeting.ts, which computes
  // this live the same way it does for EVT-013). The mirrored good-direction
  // rewards for staying well above/below these thresholds are NOT events —
  // they're automatic, no-decision-needed bonuses applied by
  // lib/social-thresholds.ts, since "you're doing great, here's a bonus"
  // doesn't need a decision prompt the way a crisis response does.

  {
    id: "EVT-017",
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Public Trust Erosion — Warning Signs",
    day: 3,
    category: "Public Communication & Risk Messaging",
    deadlineType: "SOFT",
    deadlineWindowHours: 5,
    reminderAtHours: 2.5,
    deadlineWindowDesc: "5h window; reminder at 2.5h",
    scope: "REGIONAL",
    isAnchor: false,
    narrativeMarkdown:
      "Independent polling shows public trust in your regional office's pandemic response has slipped below 45/100. Local media are running skeptical coverage of your messaging; community leaders report growing \"compliance fatigue\" at town halls. Nothing has broken yet — but it's the kind of trend line that's easy to ignore until it isn't.",
    decisionPromptMarkdown:
      "Choose a communication response: a genuine listening/engagement push, a straight messaging refresh, minimal acknowledgment, or no response.",
    minRationaleWords: 150,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Launch a genuine listening campaign — town halls, community leader briefings, and a public dashboard of what you're doing and why.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 400_000 },
        impactDesc: "Costs $400K to run properly. The only option that actually engages the specific complaints driving the number down, rather than just restating the existing message louder.",
      },
      {
        label: "B",
        text: "Refresh public messaging — clearer language, more frequent updates — without new engagement infrastructure.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 150_000 },
        impactDesc: "Costs $150K in comms production. Cheaper and faster than real engagement, and it helps some, but doesn't address why people stopped listening in the first place.",
      },
      {
        label: "C",
        text: "Brief acknowledgment in a routine press update; no dedicated response.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and no real signal that you noticed. A slipping trust number that gets a one-line mention tends to keep slipping.",
      },
      {
        label: "D",
        text: "No response — treat it as noise in the polling.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — but trust warnings that get ignored are exactly how regions end up at the next stage of this.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose public trust index drops below 45.",
    consequencesJson: {
      optimal: "Genuine engagement — trust stabilizes and partially recovers.",
      adequate: "Messaging refresh — trust stabilizes, no recovery.",
      inadequate: "Minimal acknowledgment — trust continues to slip.",
      critical: "Ignored — trust keeps falling, and political tension ticks up as the story spreads.",
    },
    modelDeltaDesc: "Optimal -> trust +8. Adequate -> trust +3. Inadequate -> trust -3. Critical -> trust -6, political tension +5.",
    modelDeltaJson: {
      OPTIMAL: [{ field: "publicTrustIndex", region: "SELF", delta: 8 }],
      ADEQUATE: [{ field: "publicTrustIndex", region: "SELF", delta: 3 }],
      INADEQUATE: [{ field: "publicTrustIndex", region: "SELF", delta: -3 }],
      CRITICAL_FAILURE: [
        { field: "publicTrustIndex", region: "SELF", delta: -6 },
        { field: "politicalTensionIndex", region: "SELF", delta: 5 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: false,
  ...
  },
  {
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Public trust in your regional office has fallen below 30/100. Compliance with guidance is visibly dropping in surveyed districts. A local outlet has published a \"why should we listen to WHO\" op-ed that's being widely shared. This is no longer a trend line — it's an active credibility crisis.",
    decisionPromptMarkdown:
      "Choose a crisis-communication response: a full accountability response, a targeted correction, damage control only, or no response.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Full accountability response — name what went wrong, commit to specific fixes, and open a standing public feedback channel.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 900_000 },
        impactDesc: "Costs $900K to stand up the accountability process and the ongoing feedback channel. Expensive and politically uncomfortable, but it's the only option that credibly reverses a credibility crisis rather than just managing it.",
      },
      {
        label: "B",
        text: "Targeted correction — address the specific op-ed's claims directly, without a broader accountability process.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 400_000 },
        impactDesc: "Costs $400K. Cheaper than a full response and stops the immediate bleeding, but doesn't rebuild trust beyond this one incident.",
      },
      {
        label: "C",
        text: "Standard damage control — a defensive statement, no substantive change.",
        suggestedTier: "INADEQUATE",
        cost: { fund: 100_000 },
        impactDesc: "Costs $100K. Cheap, and it reads that way — a defensive statement on a credibility crisis usually deepens it.",
      },
      {
        label: "D",
        text: "No response.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — and at 30/100 trust, silence is its own answer. Compliance keeps dropping, and so does everyone's patience with your office.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose public trust index drops below 30.",
    consequencesJson: {
      optimal: "Full accountability — trust recovers meaningfully, credibility partially restored.",
      adequate: "Targeted correction — trust stabilizes, no recovery.",
      inadequate: "Damage control only — trust keeps falling, political tension rises.",
      critical: "No response — trust collapses further, unrest risk rises, happiness takes a hit region-wide.",
    },
    modelDeltaDesc: "Optimal -> trust +12. Adequate -> trust +4. Inadequate -> trust -8, political tension +8. Critical -> trust -12, fund -$500K (unrest response costs), happiness -5.",
    modelDeltaJson: {
      OPTIMAL: [{ field: "publicTrustIndex", region: "SELF", delta: 12 }],
      ADEQUATE: [{ field: "publicTrustIndex", region: "SELF", delta: 4 }],
      INADEQUATE: [
        { field: "publicTrustIndex", region: "SELF", delta: -8 },
        { field: "politicalTensionIndex", region: "SELF", delta: 8 },
      ],
      CRITICAL_FAILURE: [
        { field: "publicTrustIndex", region: "SELF", delta: -12 },
        { field: "fundRemaining", region: "SELF", delta: -500_000 },
        { field: "populationHappinessIndex", region: "SELF", delta: -5 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: false,
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Public Trust Crisis — Total Breakdown",
    day: 5,
    category: "Public Communication & Risk Messaging",
    deadlineType: "HARD",
    deadlineWindowHours: 3,
    reminderAtHours: null,
    deadlineWindowDesc: "3-hour window. Non-response = recorded as Option D (no response).",
    scope: "REGIONAL",
    isAnchor: false,
    narrativeMarkdown:
      "Public trust in your regional office has collapsed below 15/100. Guidance compliance has effectively broken down in multiple surveyed districts. Aid convoys report harassment at checkpoints. This is the emotionally significant moment for this region — the population has stopped believing anything your office says.",
    decisionPromptMarkdown:
      "Emergency response required: an all-in rebuilding effort, an emergency stabilization package, minimal response, or no response.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "All-in rebuilding effort — leadership change signal, third-party audit, direct community reparations program.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 2_500_000 },
        impactDesc: "Costs $2.5M — the most expensive response in this arc. At 15/100 trust, half-measures don't move the number; this is the only option built to actually work.",
      },
      {
        label: "B",
        text: "Emergency stabilization package — visible material relief (PPE, funds) paired with a public commitment, short of a full rebuild.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 1_200_000, ppeDays: 5 },
        impactDesc: "Costs $1.2M plus 5 PPE-days distributed directly to affected districts. Meaningful, but slower to rebuild trust than the full effort.",
      },
      {
        label: "C",
        text: "Minimal response — a public statement, no material commitment.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and at this stage, a statement with nothing behind it changes nothing. Transmission risk keeps climbing as checkpoint harassment disrupts supply and surveillance.",
      },
      {
        label: "D",
        text: "No response.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — and total breakdown. Aid access degrades further, and this region is now operating with essentially no public cooperation.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose public trust index drops below 15.",
    consequencesJson: {
      optimal: "All-in rebuilding — trust begins a real recovery, Rt eases as cooperation returns.",
      adequate: "Stabilization package — trust stabilizes, partial cooperation returns.",
      inadequate: "Minimal response — trust stays collapsed, Rt worsens as compliance stays broken.",
      critical: "No response — total breakdown, Rt worsens sharply, supply chain access degrades.",
    },
    modelDeltaDesc: "Optimal -> trust +18, Rt -0.05. Adequate -> trust +8. Inadequate -> Rt +0.08. Critical -> Rt +0.15, PPE -8 (checkpoint disruption), fund -$1M.",
    modelDeltaJson: {
      OPTIMAL: [
        { field: "publicTrustIndex", region: "SELF", delta: 18 },
        { field: "rt", region: "SELF", delta: -0.05 },
      ],
      ADEQUATE: [{ field: "publicTrustIndex", region: "SELF", delta: 8 }],
      INADEQUATE: [{ field: "rt", region: "SELF", delta: 0.08 }],
      CRITICAL_FAILURE: [
        { field: "rt", region: "SELF", delta: 0.15 },
        { field: "ppeDaysRemaining", region: "SELF", delta: -8 },
        { field: "fundRemaining", region: "SELF", delta: -1_000_000 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: true,
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Public Morale Warning — Fatigue Setting In",
    day: 3,
    category: "Public Communication & Risk Messaging",
    deadlineType: "SOFT",
    deadlineWindowHours: 5,
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Population happiness/morale in your region has dropped below 45/100. Surveys cite prolonged restrictions, economic strain, and general pandemic fatigue rather than any single incident. Nothing acute has happened — people are just tired, and tired populations are harder to keep engaged.",
    decisionPromptMarkdown:
      "Choose a response: targeted relief/morale investment, a lighter acknowledgment gesture, no dedicated response, or dismiss it as unavoidable.",
    minRationaleWords: 150,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Targeted relief investment — economic support measures paired with a genuine \"we see you\" public communication push.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 350_000 },
        impactDesc: "Costs $350K in relief measures. Addresses the actual driver (fatigue from prolonged strain) rather than just the messaging around it.",
      },
      {
        label: "B",
        text: "A visible but smaller morale gesture — extended support hours, a public appreciation campaign for frontline compliance.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 120_000 },
        impactDesc: "Costs $120K. Cheaper, and it helps a little, but doesn't touch the underlying economic strain driving the fatigue.",
      },
      {
        label: "C",
        text: "No dedicated response — happiness dips are expected this deep into a sustained crisis.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and no acknowledgment. Expected or not, an unaddressed dip tends to keep dipping.",
      },
      {
        label: "D",
        text: "Dismiss the survey data publicly as overblown.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — but publicly dismissing what people are telling you tends to read as contempt, not confidence.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose population happiness index drops below 45.",
    consequencesJson: {
      optimal: "Relief investment — happiness stabilizes and partially recovers.",
      adequate: "Morale gesture — happiness stabilizes, no recovery.",
      inadequate: "No response — happiness continues to slip.",
      critical: "Dismissed publicly — happiness drops further, political tension rises as the dismissal circulates.",
    },
    modelDeltaDesc: "Optimal -> happiness +8. Adequate -> happiness +3. Inadequate -> happiness -3. Critical -> happiness -6, political tension +5.",
    modelDeltaJson: {
      OPTIMAL: [{ field: "populationHappinessIndex", region: "SELF", delta: 8 }],
      ADEQUATE: [{ field: "populationHappinessIndex", region: "SELF", delta: 3 }],
      INADEQUATE: [{ field: "populationHappinessIndex", region: "SELF", delta: -3 }],
      CRITICAL_FAILURE: [
        { field: "populationHappinessIndex", region: "SELF", delta: -6 },
        { field: "politicalTensionIndex", region: "SELF", delta: 5 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Public Morale Collapse — Unrest Risk",
    day: 4,
    category: "Public Communication & Risk Messaging",
    deadlineType: "SOFT",
    deadlineWindowHours: 4,
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Population happiness has fallen below 30/100. Small protests have been reported outside two regional health facilities. Frontline healthcare workers report rising hostility from the public they're trying to help. This is starting to look like genuine unrest risk, not just fatigue.",
    decisionPromptMarkdown:
      "Choose a response: a substantial relief + de-escalation package, a moderate relief measure, security-only response, or no response.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Substantial relief package paired with direct de-escalation outreach to protest organizers and affected communities.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 800_000 },
        impactDesc: "Costs $800K. The only option that treats this as what it is — a legitimate grievance, not a security problem — and addresses both the material strain and the anger.",
      },
      {
        label: "B",
        text: "Moderate relief measure without dedicated de-escalation outreach.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 350_000 },
        impactDesc: "Costs $350K. Helps the material strain somewhat, but leaves the anger driving the protests unaddressed.",
      },
      {
        label: "C",
        text: "Security-focused response at the affected facilities, no relief measures.",
        suggestedTier: "INADEQUATE",
        cost: { fund: 150_000 },
        impactDesc: "Costs $150K in additional security. Protects facilities in the short term, but treating unrest as a security problem rather than a grievance tends to make the underlying anger worse.",
      },
      {
        label: "D",
        text: "No response.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — and no response to reported unrest at health facilities is exactly how a small protest becomes a bigger one.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose population happiness index drops below 30.",
    consequencesJson: {
      optimal: "Relief + de-escalation — happiness recovers meaningfully, unrest risk eases.",
      adequate: "Relief only — happiness stabilizes, unrest risk unchanged.",
      inadequate: "Security only — happiness keeps falling, political tension rises.",
      critical: "No response — happiness drops further, HCW surge capacity takes a hit as frontline morale breaks.",
    },
    modelDeltaDesc: "Optimal -> happiness +12. Adequate -> happiness +4. Inadequate -> happiness -8, political tension +8. Critical -> happiness -12, HCW surge -6%.",
    modelDeltaJson: {
      OPTIMAL: [{ field: "populationHappinessIndex", region: "SELF", delta: 12 }],
      ADEQUATE: [{ field: "populationHappinessIndex", region: "SELF", delta: 4 }],
      INADEQUATE: [
        { field: "populationHappinessIndex", region: "SELF", delta: -8 },
        { field: "politicalTensionIndex", region: "SELF", delta: 8 },
      ],
      CRITICAL_FAILURE: [
        { field: "populationHappinessIndex", region: "SELF", delta: -12 },
        { field: "hcwSurgePct", region: "SELF", delta: -6 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: false,
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Public Morale Crisis — Workforce Breakdown",
    day: 5,
    category: "Public Communication & Risk Messaging",
    deadlineType: "HARD",
    deadlineWindowHours: 3,
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Population happiness has collapsed below 15/100. Multiple health facilities report healthcare workers walking off shifts, citing exhaustion and public hostility. Sustained unrest is now affecting supply movement in parts of the region. This is the workforce actually breaking, not just morale numbers on a survey.",
    decisionPromptMarkdown:
      "Emergency response required: an all-in workforce + relief intervention, an emergency retention package, minimal response, or no response.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "All-in intervention — emergency HCW retention bonuses, security for facilities framed as protection not confrontation, and a major public relief package.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 2_200_000 },
        impactDesc: "Costs $2.2M — the most expensive response in this arc, covering both keeping staff on shift and addressing the public anger driving them off it.",
      },
      {
        label: "B",
        text: "Emergency retention package for healthcare workers only, without a broader public relief measure.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 1_000_000 },
        impactDesc: "Costs $1.0M. Stabilizes staffing in the short term, but doesn't address why the public turned hostile in the first place.",
      },
      {
        label: "C",
        text: "Minimal response — a public statement, no material intervention.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and at this stage, staff keep walking off shift regardless of what gets said publicly.",
      },
      {
        label: "D",
        text: "No response.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — and workforce breakdown continues unaddressed, with real capacity consequences for everyone this region is trying to treat.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose population happiness index drops below 15.",
    consequencesJson: {
      optimal: "All-in intervention — HCW capacity stabilizes, happiness begins recovering.",
      adequate: "Retention package — HCW capacity stabilizes, happiness stays collapsed.",
      inadequate: "Minimal response — HCW capacity keeps eroding.",
      critical: "No response — HCW capacity drops sharply, CFR worsens as care capacity shrinks.",
    },
    modelDeltaDesc: "Optimal -> HCW surge +8%, happiness +15. Adequate -> HCW surge +4%. Inadequate -> HCW surge -6%. Critical -> HCW surge -15%, CFR multiplier +0.3.",
    modelDeltaJson: {
      OPTIMAL: [
        { field: "hcwSurgePct", region: "SELF", delta: 8 },
        { field: "populationHappinessIndex", region: "SELF", delta: 15 },
      ],
      ADEQUATE: [{ field: "hcwSurgePct", region: "SELF", delta: 4 }],
      INADEQUATE: [{ field: "hcwSurgePct", region: "SELF", delta: -6 }],
      CRITICAL_FAILURE: [
        { field: "hcwSurgePct", region: "SELF", delta: -15 },
        { field: "cfrMultiplier", region: "SELF", delta: 0.3 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: true,
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Member State Friction — Warning Signs",
    day: 3,
    category: "Institutional Authority / Public Communication",
    deadlineType: "SOFT",
    deadlineWindowHours: 5,
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Political tension between your regional office and member states has climbed above 60/100. Nothing has ruptured — but closed-door meetings are getting testier, and at least one member state has started routing requests through political channels instead of the normal technical ones.",
    decisionPromptMarkdown:
      "Choose a response: proactive diplomatic outreach, a routine check-in, no dedicated response, or let it ride.",
    minRationaleWords: 150,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Proactive diplomatic outreach — direct conversations with the friction points before they harden into positions.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 250_000 },
        impactDesc: "Costs $250K in diplomatic engagement. Addresses friction while it's still informal, before anyone's dug in on a public position.",
      },
      {
        label: "B",
        text: "A routine coordination check-in, folded into normal scheduled contact.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 80_000 },
        impactDesc: "Costs $80K. Low-key and cheap, and it helps somewhat, but doesn't specifically address what's actually causing the friction.",
      },
      {
        label: "C",
        text: "No dedicated response — some friction is normal at this stage of a crisis.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and no engagement. Friction that's not addressed tends to find a reason to escalate.",
      },
      {
        label: "D",
        text: "Ignore it — let member states raise concerns through official channels if they have any.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — but member states already routing around your normal channels read \"wait for official channels\" as being brushed off.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose political tension index rises above 60.",
    consequencesJson: {
      optimal: "Proactive outreach — tension eases.",
      adequate: "Routine check-in — tension stabilizes.",
      inadequate: "No response — tension continues to rise.",
      critical: "Ignored — tension rises further, trust erodes as member states notice the brush-off.",
    },
    modelDeltaDesc: "Optimal -> tension -12. Adequate -> tension -5. Inadequate -> tension +5. Critical -> tension +10, trust -5.",
    modelDeltaJson: {
      OPTIMAL: [{ field: "politicalTensionIndex", region: "SELF", delta: -12 }],
      ADEQUATE: [{ field: "politicalTensionIndex", region: "SELF", delta: -5 }],
      INADEQUATE: [{ field: "politicalTensionIndex", region: "SELF", delta: 5 }],
      CRITICAL_FAILURE: [
        { field: "politicalTensionIndex", region: "SELF", delta: 10 },
        { field: "publicTrustIndex", region: "SELF", delta: -5 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: false,
  ...
  },
  {
    ...
    isAnchor: false,
    narrativeMarkdown:
      "Political tension has risen above 75/100. A member state has formally suspended a scheduled surveillance-data sharing arrangement pending \"a review of the relationship.\" This is now an active standoff, not background friction, and your regional surveillance picture is getting worse for it.",
    decisionPromptMarkdown:
      "Choose a response: a formal negotiated resolution, a partial concession, hold firm without negotiating, or escalate publicly.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "Pursue a formal negotiated resolution — direct high-level talks aimed at restoring the arrangement on workable terms.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 600_000 },
        impactDesc: "Costs $600K in negotiation resources. Slower than the alternatives, but it's the only option that actually gets the data-sharing arrangement back, not just the temperature down.",
      },
      {
        label: "B",
        text: "Offer a partial concession to unblock the immediate standoff without a full renegotiation.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 250_000 },
        impactDesc: "Costs $250K. Faster than full negotiation and reduces immediate tension, but the underlying arrangement stays suspended.",
      },
      {
        label: "C",
        text: "Hold your position without initiating negotiation.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and no movement either. Surveillance data stays cut off while the standoff continues.",
      },
      {
        label: "D",
        text: "Escalate publicly — call out the suspension as obstruction.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — but a public escalation tends to make it harder for the member state to back down without losing face, which is how standoffs turn into ruptures.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose political tension index rises above 75.",
    consequencesJson: {
      optimal: "Negotiated resolution — tension eases, surveillance arrangement restored.",
      adequate: "Partial concession — tension stabilizes, arrangement stays suspended.",
      inadequate: "Held firm — tension continues, surveillance quality erodes.",
      critical: "Public escalation — tension spikes, surveillance data loss becomes prolonged.",
    },
    modelDeltaDesc: "Optimal -> tension -15, surveillance +1. Adequate -> tension -6. Inadequate -> surveillance -2. Critical -> tension +15, surveillance -4, fund -$600K.",
    modelDeltaJson: {
      OPTIMAL: [
        { field: "politicalTensionIndex", region: "SELF", delta: -15 },
        { field: "surveillanceIndex", region: "SELF", delta: 1 },
      ],
      ADEQUATE: [{ field: "politicalTensionIndex", region: "SELF", delta: -6 }],
      INADEQUATE: [{ field: "surveillanceIndex", region: "SELF", delta: -2 }],
      CRITICAL_FAILURE: [
        { field: "politicalTensionIndex", region: "SELF", delta: 15 },
        { field: "surveillanceIndex", region: "SELF", delta: -4 },
        { field: "fundRemaining", region: "SELF", delta: -600_000 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
    requiresMandatoryReview: false,
  ...
  },
  {
    ...
    isCorePath: false,
    suggestedTargetRegions: null,
    title: "Member State Rupture — Cooperation Breakdown",
    day: 5,
    category: "Institutional Authority / Public Communication",
    deadlineType: "HARD",
    deadlineWindowHours: 3,
    reminderAtHours: null,
    deadlineWindowDesc: "3-hour window. Non-response = recorded as Option D (escalate/no decision).",
    scope: "REGIONAL",
    isAnchor: false,
    narrativeMarkdown:
      "Political tension has risen above 90/100. A member state has formally suspended cooperation with your regional office pending a full review — no data sharing, no joint response coordination, and public statements distancing the government from WHO guidance. While this stands, your region is effectively cut off from WHO HQ's marketplace and from requesting emergency funding: nobody's approving anything for a region mid-rupture with its own coordinating authority.",
    decisionPromptMarkdown:
      "Emergency response required: an all-in reconciliation effort, a formal mediated de-escalation, hold position, or accept the rupture as-is.",
    minRationaleWords: 200,
    structuredOptionsJson: [
      {
        label: "A",
        text: "All-in reconciliation — DG-level engagement, a public commitment to the member state's core concern, and a rebuilt cooperation framework.",
        suggestedTier: "OPTIMAL",
        cost: { fund: 1_800_000 },
        impactDesc: "Costs $1.8M — the most expensive response in this arc, but it's the only one built to actually end a full rupture rather than just pause it.",
      },
      {
        label: "B",
        text: "Formal third-party mediation to de-escalate without a full DG-level commitment.",
        suggestedTier: "ADEQUATE",
        cost: { fund: 900_000 },
        impactDesc: "Costs $900K. Slower and less certain than direct reconciliation, but meaningfully cheaper.",
      },
      {
        label: "C",
        text: "Hold position and wait out the review period.",
        suggestedTier: "INADEQUATE",
        cost: {},
        impactDesc: "No cost — and no progress. The rupture, and the marketplace/emergency-funding lockout that comes with it, continues.",
      },
      {
        label: "D",
        text: "Accept the rupture — redirect effort to regions still cooperating.",
        suggestedTier: "CRITICAL_FAILURE",
        cost: {},
        impactDesc: "No cost — and this region is now on its own for the remainder of the simulation, with real epidemiological consequences from the lost coordination.",
      },
    ],
    triggerConditionDesc: "Adaptive — fires for any region whose political tension index rises above 90. While tension stays above 90, that region cannot buy from WHO HQ or open an emergency funding request (see app/api/market and app/api/emergency-funding).",
    consequencesJson: {
      optimal: "All-in reconciliation — tension drops well below the lockout threshold, cooperation restored.",
      adequate: "Mediated de-escalation — tension drops below the lockout threshold.",
      inadequate: "Held position — tension stays at rupture level, lockout continues.",
      critical: "Rupture accepted — tension stays at maximum, Rt worsens as coordination stays severed.",
    },
    modelDeltaDesc: "Optimal -> tension -25, trust +5. Adequate -> tension -15. Inadequate -> tension +0 (no change, lockout persists). Critical -> Rt +0.1, tension +5.",
    modelDeltaJson: {
      OPTIMAL: [
        { field: "politicalTensionIndex", region: "SELF", delta: -25 },
        { field: "publicTrustIndex", region: "SELF", delta: 5 },
      ],
      ADEQUATE: [{ field: "politicalTensionIndex", region: "SELF", delta: -15 }],
      INADEQUATE: [],
      CRITICAL_FAILURE: [
        { field: "rt", region: "SELF", delta: 0.1 },
        { field: "politicalTensionIndex", region: "SELF", delta: 5 },
      ],
    },
    noResponseFallbackTier: "CRITICAL_FAILURE",
